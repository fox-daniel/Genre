...
def clean_genre_data():
    ...
    data = pd.read_csv(
        "data/genre_lists/data_to_be_cleaned/wiki-kaggle_genres_rescrape.csv", header=0
    )
    ...
    data["match"] = (
        data.artist.apply(lambda x: x.casefold())
        != data.retrieved_clean.apply(lambda x: x.casefold())
    ).astype("int64")
    ...
    data.drop(
        ["retrieved", "genre", "retrieved_clean", "match", "genre_respell"],
        axis=1,
        inplace=True,
    )
...
def cleaning_individual_cases_1(data):
    ...
    data.genre.at[
        11935
    ] = "[rock, active rock, country, jazz standards, childrens folk]"
    data.at[
        13009, "genre"
    ...
    data.at[
        14600, "genre"
    ] = "['rock', 'pop', 'jazz', 'r&b', 'country', ' blues', 'roots']"
    data.at[10022, "genre"] = "['indie','alternative rock', 'pop punk']"
    data.at[
        13433, "genre"
    ] = "['blues', 'roots', ' rock&roll', 'americana', 'rhythm&blues', 'alternative']"
    ...
    data.at[
        8968, "genre"
    ] = "['rock', 'folk_rock', 'alternative_rock', 'acoustic_rock', 'reggae']"

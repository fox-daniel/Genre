...
class LoadGenreData:
    ...
    def get_list_of_genres(self):
        ...
        self.list_of_genres = [
            label for artists_labels in self.list_of_genres for label in artists_labels
        ]
    ...
    def get_sparse_X_vector(self):
        ...
        vec = CountVectorizer(
            vocabulary=dict_genre_to_id
        )  # uses scipy.sparse.csr_matrix representation
    ...
    def get_dict_genre_to_id(self):
        ...
        dict_genre_to_id = dict(
            zip(self.list_of_genres, range(len(self.list_of_genres)))
        )
    ...
    def get_dict_id_to_genre(self):
        ...
        self.list_of_genres = self.get_list_of_genres()
        dict_genre_to_id = dict(
            zip(range(len(self.list_of_genres)), self.list_of_genres)
        )
        return dict_genre_to_id
...
def to_strings(string):
    ...
    for x in L:
        L_new.append(
            x.replace(" ", "_").lstrip("_").rstrip("_").strip("!").replace("+", "_")
        )
    while str("") in L_new:
        L_new.remove("")
